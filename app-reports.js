...
async function renderQuots(){
  ...
  if(!['sales','manager','admin'].includes(ME.role)){
  ...
  const {data:quots,error}=await rowsOf(()=>sb.from('quotations')
    .select('*, leads!quotations_lead_id_fkey(ref_id, customer_name)').order('created_at',{ascending:false}).order('id'));
