...
const QT={
  ...
  vat10:'អាករលើតម្លៃលបន្ថែម/VAT 10% :', grand:'សរុបរួមទាំងអាករ/Grand Total :',
...
function imgFor(model,kind,brand,size){
  if(model){
    if(/^SUN-\d+K-SG05LP[13]-EU-SM2$/.test(model))return 'img/deye-sg05lp-v3.png';
  ...
  if(kind==='Inverter'&&size){
    ...
    if(isBrand('inverter','Deye',brand)
       &&(DEYE_SG05LP['1'].includes(n)||DEYE_SG05LP['3'].includes(n)))return 'img/deye-sg05lp-v3.png';
...
function quoteHtml(q,l,c){
  ...
  /* A portrait photo at the same height as a square one reads as half the
     size - the Deye inverter stands 609x937 beside an 11mm square panel. TALL
     lets those use more height so the three products look alike on the sheet. */
  const TALL=/deye-sg05lp/;
  const cell=(base,img)=>img?`<img class="${TALL.test(img)?'tall':''}" src="${esc((base||'')+img)}" alt="" onerror="this.remove()">`:'';
...
<style>
  ...
  .items td.im img{max-width:92%;max-height:10mm;display:inline-block}
  /* 14mm against the square photos' 11: a 609x937 inverter needs the extra
     height to read as the same size as an 11mm square panel beside it */
  .items td.im img.tall{max-height:14mm}
  ...
  /* Their sheet leaves the area to the left of the totals open - no rules
  ...
  /* the savings figures and the notes sit side by side at the foot of page
     ...
     margin-top is 8px: it sat right under the total and read as cramped, and
     half of what it was given went back to the inverter photo. Measure page one with .vatbox hidden when judging whether it
  ...
  .lower{display:flex;gap:5mm;align-items:stretch;margin-top:8px}
...
<div class="sheet" contenteditable="true">
  ...
  <table class="money">
    ...
    <tr id="r-d1"><td class="open"></td><td class="dlab"><span class="box lbl" contenteditable="true">${QT.discount}</span></td>
        <td class="v"><span class="box num" id="f-d1" contenteditable="true"></span></td></tr>
    <tr id="r-d2"><td class="open"></td><td class="dlab"><span class="box lbl" contenteditable="true"></span></td>
        <td class="v"><span class="box num" id="f-d2" contenteditable="true"></span></td></tr>
    <tr id="r-net"><td class="open"></td><td class="tot">${QT.netTotal}</td><td class="v paid" id="o-net"></td></tr>
